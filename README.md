...
| Auth Service | 8085 | PostgreSQL | JWT authentication and roles |
...
| Auth Swagger | http://localhost:8085/swagger-ui.html | admin@fleet.local / Admin123! |
...
```
POST   /api/auth/register          # Register CUSTOMER_ADMIN account
POST   /api/auth/login             # Login and receive JWT
GET    /api/auth/me                # Current user, requires Bearer token
```

Default admin account in Docker:
```
email: admin@fleet.local
password: Admin123!
```

Use authenticated business APIs through the gateway with:
```
